...
%
% Copyright (C) 2015-2023 by USTC TeX Users Group <https://github.com/szcutug>
%
...
本模板 \cls{szcuthesis} 是苏州城市学院本科生和研究生学位论文的 \LaTeX{}
...
其前身是苏州城市学院本科论文模板（作者 XPS，最后维护 ywg）
和苏州城市学院研究生论文模板（作者 Liuqs，主要维护 Liuqs、Guolicai）。
...
下载地址：
\begin{itemize}
  \item 主要地址：\url{https://github.com/szcutug/szcuthesis/releases}
  \item 学校镜像：\url{https://git.lug.szcu.edu.cn/szcutug/szcuthesis}
  \item 研究生院网站（版本较旧）：
    \url{https://gradschool.szcu.edu.cn/column/65}
\end{itemize}

用户在使用 \pkg{szcuthesis} 模板前，应先阅读学校的《撰写手册》等规范。
如果在使用的过程中遇到问题，可以阅读
\href{https://github.com/szcutug/szcuthesis/wiki}{常见问题}，
或者在 \href{https://github.com/szcutug/szcuthesis/issues}{GitHub Issues}
中反馈。
...
\begin{table}[htb]
  ...
  \begin{tabular}{lll}
    ...
    类别     & 文件                      & 说明                         \\
    \midrule
    模板文件 & \file{szcuthesis.cls}     & 模板文件                     \\
             & \file{szcuthesis-*.bst}   & \BibTeX{} 参考文献表格式     \\
             & \file{szcuthesis-*.bbx}   & BibLaTeX 参考文献表格式      \\
             & \file{szcuthesis-*.cbx}   & BibLaTeX 参考文献表格式      \\
             & \file{figures/szcu-*.pdf} & 校名和校徽图片               \\
    \midrule
    使用说明 & \file{szcuthesis-doc.tex} & 模板使用说明的源代码         \\
             & \file{szcuthesis-doc.pdf} & （你正在阅读的）模板使用说明 \\
    \midrule
    示例文档 & \file{main.tex}           & 主文档                       \\
             & \file{szcusetup.tex}      & 配置文件                     \\
             & \file{chapters/*.tex}     & 示例文档的各个章节           \\
             & \file{figures/}           & 放置图片的目录               \\
             & \file{bib/szcu.bib}       & \BibTeX{} 示例数据库         \\
    \midrule
...
\begin{shell}
  make
\end{shell}
编译说明文档 \file{szcuthesis-doc.pdf}：
\begin{shell}
  make doc
\end{shell}
...
\begin{latex}
  \documentclass[degree-type=professional]{szcuthesis}
\end{latex}

\DescribeOption{language}
论文全文的主要语言。可选：\opt{chinese}（默认），\opt{english}。
\begin{latex}
  \documentclass[language=english]{szcuthesis}
\end{latex}
...
\end{itemize}
\begin{latex}
  \documentclass[output=electronic]{szcuthesis}
\end{latex}
...
\end{itemize}
\begin{latex}
  \documentclass[section-style=arabic]{szcuthesis}
\end{latex}
...
\begin{latex}
  \documentclass[fontset=windows]{szcuthesis}
\end{latex}
...
封面由 \cs{maketitle} 命令生成，其中的各项信息使用 \cs{szcusetup} 命令以
key-value 的方式填写，如：
\begin{latex}
  \szcusetup{
    title  = {论文中文题目},
...
有几点说明：
\begin{itemize}
  \item \cs{szcusetup} 使用 \pkg{kvsetkeys} 机制，配置项之间不能有空行，否则会报错。
...
有的院系额外要求声明页有“评审专家签名”，可以在模板中使用 \cs{szcusetup} 命令设置
\opt{reviewer = true}。
...
对于特殊的章节，\cls{szcuthesis} 还提供了相应的环境：
\begin{itemize}
...
\DescribeOption{keywords*}
摘要的关键词应使用 \cs{szcusetup} 的接口进行设置，在录入时使用西文逗号隔开，
比如：
\begin{latex}
  \begin{abstract}
    这里是摘要。
...
\begin{latex}
  ...
  \bibliographystyle{szcuthesis-numeric}
\end{latex}
或者
\begin{latex}
  % 著者-出版年制
  \usepackage{natbib}
  \bibliographystyle{szcuthesis-authoryear}
\end{latex}
...
\begin{latex}
  \usepackage[style=szcuthesis-authoryear]{biblatex}
  \addbibresource{bib/szcu.bib}
\end{latex}
...
\begin{latex}
  \usepackage[bibstyle=szcuthesis-numeric,
              citestyle=szcuthesis-inline]{biblatex}
\end{latex}
